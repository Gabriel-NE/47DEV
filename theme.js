...
var ScrollCarousel = class extends HTMLElement {
  ...
  get scrollStep() {
    const attr = this.getAttribute("scroll-step");
    return attr ? parseInt(attr, 10) : this.groupCells;
  }
  /**
   * -------------------------------------------------------------------------------------------------------------------
   * PUBLIC API (METHODS)
  ...
  previous({ instant = false } = {}) {
    ...
    this.select(Math.max(__privateGet(this, _targetIndex2) - this.scrollStep, 0), { instant });
  }
  next({ instant = false } = {}) {
    // this.select(Math.min(__privateGet(this, _targetIndex2) + this.groupCells, this.cells.length - 1), { instant });
    this.select(Math.min(__privateGet(this, _targetIndex2) + this.scrollStep, this.cells.length - 1), { instant });
  }
...
var LoopingScrollCarousel = class LoopingScrollCarousel extends ScrollCarousel {
  next({ instant = false } = {}) {
    const nextIndex = this.selectedIndex + this.scrollStep;
    if (nextIndex >= this.cells.length) {
      this.select(0, { instant });
    } else {
      super.next({ instant }); // fallback to original behavior
    }
  }

  previous({ instant = false } = {}) {
    const prevIndex = this.selectedIndex - this.scrollStep;
    if (prevIndex < 0) {
      this.select(this.cells.length - 1, { instant });
    } else {
      super.previous({ instant }); // fallback to original behavior
    }
  }
}
...
if (!window.customElements.get("looping-scroll-carousel")) {
  window.customElements.define("looping-scroll-carousel", LoopingScrollCarousel);
}
      ...
      case "open":
        ...
        if (oldValue === null && (newValue === "" || newValue === "immediate")) {
          ...
          console.log(window.innerWidth);
            ...
            if (this.tagName === "HEADER-SEARCH" && window.innerWidth > 1024) {
            document.getElementsByClassName("header__primary-nav")[0].style.visibility = "hidden";
            document.getElementsByClassName("header__secondary-nav")[0].style.visibility = "hidden";
            document.getElementsByClassName("header__logo")[0].style.visibility = "hidden";
            }
          ...
          hideTransitionPromise.then(() => {
            ...
            console.log(window.innerWidth);
            if(this.tagName === "HEADER-SEARCH" && window.innerWidth > 1024) {
              document.getElementsByClassName("header__primary-nav")[0].style.visibility = "visible";
              document.getElementsByClassName("header__secondary-nav")[0].style.visibility = "visible";
              document.getElementsByClassName("header__logo")[0].style.visibility = "visible"
            };
...
onRerender_fn = function(event) {
  ...
  if (!this.hasAttribute("allow-partial-rerender") || event.detail.productChange) {
    ...
    const blockTypes = ["sku", "badges", "quantity-selector", "volume-pricing", "price", "payment-terms", "variant-picker", "inventory", "buy-buttons", "pickup-availability", "liquid","materiales"];
...
doPredictiveSearch_fn = async function() {
  ...
  document.documentElement.dispatchEvent(new CustomEvent("theme:loading:end", { bubbles: true }));
  const hiddenTab = document.querySelector(".predictive-search__resource-item[hidden]");
  hiddenTab.removeAttribute("hidden");
  const collectionTabs = hiddenTab.querySelector(".predictive-search__collections");
  collectionTabs.style.color = "black";
};
/**
...
var HeaderSearch = class extends DialogElement {
  ...
  createLeaveAnimationControls() {
    return timeline9([
      [this.getShadowPartByName("overlay"), { opacity: [1, 0] }, { duration: 0.2, easing: [0.645, 0.045, 0.355, 1] }],
      [this.getShadowPartByName("content"), 
        { opacity: [1, 0], transform: ["translateX(0)", "translateX(100%)"] }, 
        { duration: 0.2, at: "<", easing: [0.645, 0.045, 0.355, 1] }]
    ]);
  }
};
